#!/usr/bin/env python3
"""
Desktop YouTube Translator for Dad
===================================
Captures Mac system audio via BlackHole, translates English→Chinese,
and plays the translation through a specified audio output (e.g., Bluetooth earbuds).

Requirements:
1. BlackHole virtual audio driver installed (brew install blackhole-2ch)
2. Mac Audio MIDI Setup configured with Multi-Output Device
3. Bluetooth earbuds connected to Mac
...
def find_blackhole_device():
    ...
    for i, device in enumerate(devices):
        if 'blackhole' in device['name'].lower() and device['max_input_channels'] > 0:
            print(f"✅ Found BlackHole input device: {device['name']} (index {i})")
            return i
    return None
...
async def main():
    ...
    print("=" * 60)
    print("🎬 Dad's YouTube Translator - Desktop Version")
    print("=" * 60)
    
    # List devices
    list_audio_devices()
    
    # Find BlackHole input
    blackhole_device = find_blackhole_device()
    if blackhole_device is None:
        print("\n❌ BlackHole not found! Please:")
    ...
    try:
        ...
        async with deepgram.listen.v1.connect(
            ...
            # Stream audio from BlackHole to Deepgram
